...
typedef struct {
    u128 p, ni, R2, one;
    /* For R < p < 2R (R=2^64), centered field elements have a
       64-bit magnitude.  recip64=floor(2^128/p) supports a very cheap
       Barrett reduction of their 128-bit product. */
    u64 recip64;
    int fast65;
    int fast2;
} Mont128;
...
static void m128_init(Mont128 *m, u128 p) {
    m->p = p;
    m->fast65 = (p > ((u128)1 << 64) && p < ((u128)1 << 65));
    /* The production tiers (70 and 90 bits) use a two-limb integrated
       Montgomery reduction.  Keeping a little headroom below 2^128 makes
       the single final subtraction and carry bounds explicit. */
    m->fast2 = (p < ((u128)1 << 126));
    m->recip64 = m->fast65 ? (u64)(((u128)-1) / p) : 0;
...
static inline u128 mred128_ref(u256 T, const Mont128 *m) {
    u128 q = T.lo * m->ni;
...
/* REDC in radix 2^64, specialized to a two-limb modulus.  The older
   formulation first formed a 128-bit q and then two generic 128x128 wide
   products.  This integrated form needs exactly eight 64x64 multiplies for
   a complete field product (four for T=a*b and two per reduction limb).
   It is valid at both the judge tier and the 90-bit frontier. */
static inline u128 mred128_2limb(u256 T, const Mont128 *m) {
    const u64 n0 = (u64)m->p;
    const u64 n1 = (u64)(m->p >> 64);
    const u64 n0i = (u64)m->ni;
    u64 t0 = (u64)T.lo;
    u64 t1 = (u64)(T.lo >> 64);
    u64 t2 = (u64)T.hi;
    u64 t3 = (u64)(T.hi >> 64);
    ...
    u128 qn0 = (u128)q * n0;
    u128 qn1 = (u128)q * n1;
    u128 z = (u128)t0 + (u64)qn0;
    u128 s = (u128)t1 + (u64)(qn0 >> 64) + (u64)qn1 + (u64)(z >> 64);
    u64 r1 = (u64)s;
    s = (u128)t2 + (u64)(qn1 >> 64) + (u64)(s >> 64);
    u64 r2 = (u64)s;
    s = (u128)t3 + (u64)(s >> 64);
    u64 r3 = (u64)s;
    ...
    qn1 = (u128)q * n1;
    z = (u128)r1 + (u64)qn0;
    s = (u128)r2 + (u64)(qn0 >> 64) + (u64)qn1 + (u64)(z >> 64);
    u64 o0 = (u64)s;
    s = (u128)r3 + (u64)(qn1 >> 64) + (u64)(s >> 64);
    u64 o1 = (u64)s;
    u128 out = ((u128)o1 << 64) | o0;
    return out >= m->p ? out - m->p : out;
}

static inline u128 mred128(u256 T, const Mont128 *m) {
    return __builtin_expect(m->fast2, 1) ? mred128_2limb(T, m)
                                         : mred128_ref(T, m);
}
...
static inline u128 mulmod128_mont(u128 a, u128 b, const Mont128 *mt) {
    if (__builtin_expect(mt->fast65, 1)) {
        const u128 p = mt->p;
        const u128 half = p >> 1;
        int neg = 0;
        u64 aa, bb;
        if (a > half) { aa = (u64)(p - a); neg ^= 1; }
        else aa = (u64)a;
        if (b > half) { bb = (u64)(p - b); neg ^= 1; }
        else bb = (u64)b;

        /* aa*bb < p^2/4 < 2^128 at this tier.  If x=xh*R+xl and
           mu=floor(R^2/p), floor(x*mu/R^2) is obtained with two
           64x64 multiplies; its error is at most one here. */
        u128 x = (u128)aa * bb;
        u64 xl = (u64)x, xh = (u64)(x >> 64);
        u64 mu = mt->recip64;
        u128 cross = (u128)xh * mu + ((u128)xl * mu >> 64);
        u64 q = (u64)(cross >> 64);
        u128 qp = (u128)q * (u64)p + ((u128)q << 64);
        u128 r = x - qp;
        if (r >= p) r -= p;
        if (neg && r) r = p - r;
        return r;
    }
    /* Mixed-domain Montgomery multiplication: REDC((aR)*b) = a*b.
       Converting both inputs and converting the result back, as upstream
       did, performs four reductions; one conversion plus one mixed product
       performs two. */
    return mm128(toM128(a, mt), b, mt);
}

static inline u64 mulmag65(u64 a, int aneg, u64 b, int bneg,
                           int *outneg, const Mont128 *mt) {
    u128 x = (u128)a * b;
    u64 xl = (u64)x, xh = (u64)(x >> 64), mu = mt->recip64;
    u128 cross = (u128)xh * mu + ((u128)xl * mu >> 64);
    u64 q = (u64)(cross >> 64);
    u128 p = mt->p;
    u128 r = x - ((u128)q * (u64)p + ((u128)q << 64));
    if (r >= p) r -= p;
    int neg = aneg ^ bneg;
    if (r > (p >> 1)) { r = p - r; neg ^= 1; }
    *outneg = neg;
    return (u64)r;
}
...
static u128 powmod128_mont(u128 a, u128 e, const Mont128 *mt) {
    if (__builtin_expect(mt->fast65, 1)) {
        u128 r = 1, b = a % mt->p;
        while (e > 0) {
            if (e & 1) r = mulmod128_mont(r, b, mt);
            b = mulmod128_mont(b, b, mt);
            e >>= 1;
        }
        return r;
    }
...
/* Same exponent on independent bases.  Keeping the bases in flight together
   breaks the long mul-latency dependency chain of scalar powering. */
static void powmod128_batch(u128 *out, const u128 *in, int n, u128 e,
                            const Mont128 *mt) {
    u128 r[64], b[64];
    for (int j = 0; j < n; j++) { r[j] = 1; b[j] = in[j]; }
    while (e > 0) {
        if (e & 1) {
            for (int j = 0; j < n; j++) r[j] = mulmod128_mont(r[j], b[j], mt);
        }
        for (int j = 0; j < n; j++) b[j] = mulmod128_mont(b[j], b[j], mt);
        e >>= 1;
    }
    for (int j = 0; j < n; j++) out[j] = r[j];
}

static void sqrtmod128_batch(u128 *roots, unsigned char *ok,
                             const u128 *ns, int n, u128 p, u128 sqrtm1,
                             const Mont128 *mt) {
    if (n <= 0) return;
    u128 e = ((p & 3) == 3) ? ((p + 1) >> 2) : ((p + 3) >> 3);
    powmod128_batch(roots, ns, n, e, mt);
    for (int j = 0; j < n; j++) {
        if (ns[j] == 0) { roots[j] = 0; ok[j] = 1; continue; }
        u128 x2 = mulmod128_mont(roots[j], roots[j], mt);
        if (x2 == ns[j]) { ok[j] = 1; continue; }
        if ((p & 3) != 3) {
            roots[j] = mulmod128_mont(roots[j], sqrtm1, mt);
            x2 = mulmod128_mont(roots[j], roots[j], mt);
        }
        ok[j] = (x2 == ns[j]);
    }
}
...
static u128 invert128_mont(u128 a, u128 p, const Mont128 *mt) {
    if (__builtin_expect(mt->fast65, 0) && 0) {
        /* Binary extended GCD, retaining coefficients modulo p.  This is
           substantially cheaper at 65 bits than another Fermat power. */
        a %= p;
        if (a == 0) return 0;
        u128 u = a, v = p, x1 = 1, x2 = 0;
        while (u != 1 && v != 1) {
            while ((u & 1) == 0) {
                u >>= 1;
                x1 = (x1 & 1) ? ((x1 + p) >> 1) : (x1 >> 1);
            ...
            if (u >= v) {
                u -= v;
                x1 = x1 >= x2 ? x1 - x2 : p - x2 + x1;
            } else {
                v -= u;
                x2 = x2 >= x1 ? x2 - x1 : p - x1 + x2;
            }
        }
        return u == 1 ? x1 : x2;
    }
...
static int x16_y_predicts_nonsplit_sqrt128(u128 p, u128 y, u128 y2,
                                            u128 sqrtm1, const Mont128 *mt) {
    u128 f1 = submod128(y2, 2, p);
    u128 four_y = addmod128(addmod128(y, y, p), addmod128(y, y, p), p);
    u128 f2 = addmod128(submod128(y2, four_y, p), 2, p);
    u128 f = mulmod128_mont(f1, f2, mt), root;
    if (f == 0) return 0;
    return !sqrtmod_p5_128(&root, f, p, sqrtm1, mt);
}
...
static int x16_montgomery_A_nonsplit128(u128 *Ao, u128 *xPo,
    ...
    for (;;) {
        ...
        u128 y2 = mulmod128_mont(y, y, mt);
        if (!x16_y_predicts_nonsplit_sqrt128(p, y, y2, sqrtm1, mt)) continue;
        ...
        /* Keep the quadratic roots projective.  If
             x = n/(2qa), then x/(x-y) = n/(n-2qa*y).
           This lets the common A denominator and both marked-point
           denominators share one inversion, instead of first inverting
           2qa and then doing one batch inversion per root. */
        u128 twoqa = addmod128(qa, qa, p);
        u128 twoqa_y = mulmod128_mont(twoqa, y, mt);
        u128 rnum[2] = {
            submod128(sd, qb, p),
            submod128(submod128(0, sd, p), qb, p)
        };
        u128 ym1 = submod128(y, 1, p);
        u128 ym1_2 = mulmod128_mont(ym1, ym1, mt);
        u128 denA = mulmod128_mont(4, mulmod128_mont(ym1_2, ym1_2, mt), mt);
        u128 vals[3] = {
            denA,
            submod128(rnum[0], twoqa_y, p),
            submod128(rnum[1], twoqa_y, p)
        };
        u128 invs[3];
        if (!invert_batch128(invs, vals, 3, p, mt)) continue;
        u128 A = mulmod128_mont(x16_A_numerator_from_y128(y, p, mt), invs[0], mt);
        if (A <= 2 || A >= p - 2) continue;
        u128 xp0 = mulmod128_mont(rnum[0], invs[1], mt);
        u128 xp1 = mulmod128_mont(rnum[1], invs[2], mt);
        if (xp0 == 0 || xp1 == 0) continue;
        *Ao = A;
        *xPo = xp0;
        *pending_A = A;
        *pending_xP = xp1;
...
/* Fill a queue of distinct nonsplit X1(16) curves.  Both roots of the
   auxiliary quadratic give the same A and, in the nonsplit (cyclic 2-Sylow)
   family, the same halving depth.  Keep one marked point per A. */
static int x16_fill_nonsplit_batch128(u128 *As, u128 *xPs, int cap,
                                      Rng *rng, u128 p, u128 rand_mask,
                                      u128 sqrtm1, const Mont128 *mt) {
    enum { MAXY = 32 };
    if (cap > MAXY) cap = MAXY;
    int target = cap;
    u128 ys[MAXY], r0s[MAXY];
    u128 vals[2 * MAXY], invs[2 * MAXY];
    int ny = 0;
    while (ny < target) {
        u128 y = rand_below128(rng, p, rand_mask);
        if (y == 0) continue;
        ...
        u128 qc = submod128(1, y, p);
        u128 D = submod128(mulmod128_mont(qb, qb, mt),
                           mulmod128_mont(addmod128(qa, qa, p),
                                         addmod128(qc, qc, p), mt), p);
        u128 sd;
        if (!sqrtmod_p5_128(&sd, D, p, sqrtm1, mt)) continue;
        u128 twoqa_y = mulmod128_mont(addmod128(qa, qa, p), y, mt);
        u128 r0 = submod128(sd, qb, p);
        u128 ym1 = submod128(y, 1, p);
        u128 ym1_2 = mulmod128_mont(ym1, ym1, mt);
        u128 denA = mulmod128_mont(4, mulmod128_mont(ym1_2, ym1_2, mt), mt);
        u128 dx0 = submod128(r0, twoqa_y, p);
        if (denA == 0 || dx0 == 0) continue;
        ys[ny] = y; r0s[ny] = r0;
        vals[2*ny] = denA;
        vals[2*ny+1] = dx0;
        ny++;
    }
    if (!invert_batch128(invs, vals, 2 * ny, p, mt)) return 0;
    int nout = 0;
    for (int i = 0; i < ny; i++) {
        u128 A = mulmod128_mont(x16_A_numerator_from_y128(ys[i], p, mt),
                                invs[2*i], mt);
        if (A <= 2 || A >= p - 2) continue;
        u128 xp0 = mulmod128_mont(r0s[i], invs[2*i+1], mt);
        if (xp0 == 0) continue;
        As[nout] = A; xPs[nout++] = xp0;
    }
    return nout;
}

/* Production Montgomery-domain X1(16) path.  The statistics modes retain
   their historical ordinary-residue interfaces; this path converts only the
   random source and the final answer. */
static int g_domain_window3 = 0;
static int g_domain_slide3 = 0;
static int g_force_affine = 0;
static int g_use_walker = 0;

static inline u128 pow128_domain_binary(u128 a, u128 e, const Mont128 *mt) {
    u128 r = mt->one;
    while (e > 1) {
        if (e & 1) r = mm128(r, a, mt);
        a = mm128(a, a, mt);
        e >>= 1;
    }
    return e ? mm128(r, a, mt) : r;
}

static inline u128 pow128_domain_window3(u128 a, u128 e, const Mont128 *mt) {
    u128 tab[8];
    tab[0] = mt->one;
    tab[1] = a;
    for (int i = 2; i < 8; i++) tab[i] = mm128(tab[i - 1], a, mt);
    int bits = bitlen128(e);
    int first = bits % 3;
    if (!first) first = 3;
    int shift = bits - first;
    unsigned idx = (unsigned)((e >> shift) & (((u128)1 << first) - 1));
    u128 r = tab[idx];
    while (shift) {
        r = mm128(r, r, mt);
        r = mm128(r, r, mt);
        r = mm128(r, r, mt);
        shift -= 3;
        idx = (unsigned)((e >> shift) & 7);
        if (idx) r = mm128(r, tab[idx], mt);
    }
    return r;
}

static inline u128 pow128_domain_slide3(u128 a, u128 e, const Mont128 *mt) {
    u128 odd[4];
    odd[0] = a;
    u128 a2 = mm128(a, a, mt);
    odd[1] = mm128(odd[0], a2, mt);
    odd[2] = mm128(odd[1], a2, mt);
    odd[3] = mm128(odd[2], a2, mt);

    int i = bitlen128(e) - 1;
    int lo = i > 1 ? i - 2 : 0;
    while (((e >> lo) & 1) == 0) lo++;
    unsigned w = (unsigned)((e >> lo) & (((u128)1 << (i - lo + 1)) - 1));
    u128 r = odd[(w - 1) >> 1];
    i = lo - 1;
    while (i >= 0) {
        if (((e >> i) & 1) == 0) {
        ...
        lo = i > 1 ? i - 2 : 0;
        while (((e >> lo) & 1) == 0) lo++;
        w = (unsigned)((e >> lo) & (((u128)1 << (i - lo + 1)) - 1));
        for (int j = lo; j <= i; j++) r = mm128(r, r, mt);
        r = mm128(r, odd[(w - 1) >> 1], mt);
        i = lo - 1;
    }
    return r;
}

static inline u128 pow128_domain(u128 a, u128 e, const Mont128 *mt) {
    if (__builtin_expect(g_domain_slide3, 0))
        return pow128_domain_slide3(a, e, mt);
    return __builtin_expect(g_domain_window3, 0)
        ? pow128_domain_window3(a, e, mt)
        : pow128_domain_binary(a, e, mt);
}

static inline int sqrt128_domain(u128 *root, u128 n, u128 p,
                                 u128 sqrtm1_m, const Mont128 *mt) {
    if (n == 0) { *root = 0; return 1; }
    u128 x;
    if ((p & 3) == 3) {
        x = pow128_domain(n, (p + 1) >> 2, mt);
        if (mm128(x, x, mt) != n) {
            /* For p=3 mod 4 this is a square root of -n. */
            *root = x;
            return 0;
        }
    } else {
        x = pow128_domain(n, (p + 3) >> 3, mt);
        if (mm128(x, x, mt) != n) {
            x = mm128(x, sqrtm1_m, mt);
            if (mm128(x, x, mt) != n) {
                /* A nonsquare leaves a +/-sqrt(-1) phase. */
                *root = x;
                return 0;
            }
        }
...
static int invert_batch128_domain(u128 *out, const u128 *vals, int n,
                                  u128 p, const Mont128 *mt) {
    u128 prefix[65];
    ...
    for (int i = 0; i < n; i++) {
        if (vals[i] == 0) return 0;
        prefix[i + 1] = mm128(prefix[i], vals[i], mt);
    }
    u128 acc = pow128_domain(prefix[n], p - 2, mt);
    for (int i = n - 1; i >= 0; i--) {
        out[i] = mm128(acc, prefix[i], mt);
        acc = mm128(acc, vals[i], mt);
    }
    return 1;
}

static inline u128 x16_A_num_domain(u128 y, u128 p, const Mont128 *mt) {
    u128 c8 = mt->one;
    c8 = addmod128(c8, c8, p);
    c8 = addmod128(c8, c8, p);
    c8 = addmod128(c8, c8, p);
    u128 c16 = addmod128(c8, c8, p);
    u128 c24 = addmod128(c16, c8, p);
    u128 c32 = addmod128(c16, c16, p);
    u128 c48 = addmod128(c32, c16, p);
    u128 num = mt->one;
    num = submod128(mm128(num, y, mt), c8, p);
    num = addmod128(mm128(num, y, mt), c24, p);
    num = submod128(mm128(num, y, mt), c32, p);
    num = addmod128(mm128(num, y, mt), c8, p);
    num = addmod128(mm128(num, y, mt), c32, p);
    num = submod128(mm128(num, y, mt), c48, p);
    num = addmod128(mm128(num, y, mt), c32, p);
    return submod128(mm128(num, y, mt), c8, p);
}

typedef struct { u128 H, X, W; } CubicPoint128;

typedef struct {
    u128 p, u0, v0, c1, c2, c3, c4;
    u128 eta, sqrtm1_m, kplus, kminus;
    CubicPoint128 cur, inc[8];
    Rng *rng;
    const Mont128 *mt;
    int ready;
} NonsplitWalker128;

static inline u128 halfmod128(u128 x, u128 p) {
    return (x & 1) ? ((x + p) >> 1) : (x >> 1);
}

static u128 cubic_eval128(const NonsplitWalker128 *w, CubicPoint128 P) {
    const Mont128 *m = w->mt;
    u128 p = w->p;
    u128 H2 = mm128(P.H, P.H, m);
    u128 HW = mm128(P.H, P.W, m);
    u128 W2 = mm128(P.W, P.W, m);
    u128 X2 = mm128(P.X, P.X, m);
    u128 a = addmod128(mm128(w->c4, H2, m), mm128(w->c3, HW, m), p);
    a = addmod128(a, mm128(w->c2, W2, m), p);
    a = submod128(a, X2, p);
    u128 b = addmod128(mm128(w->c1, W2, m),
                       mm128(addmod128(w->v0, w->v0, p),
                             mm128(P.X, P.W, m), m), p);
    return addmod128(mm128(P.H, a, m), mm128(P.W, b, m), p);
}

/* Group addition on the translated plane cubic with origin O=(0:1:0).
   The first chord finds R; the second chord through R and O has the compact
   closed form used below. */
static int cubic_add128(CubicPoint128 *out, const NonsplitWalker128 *w,
                        CubicPoint128 P, CubicPoint128 Q) {
    u128 p = w->p;
    const Mont128 *m = w->mt;
    CubicPoint128 plus = {
        addmod128(P.H, Q.H, p), addmod128(P.X, Q.X, p),
        addmod128(P.W, Q.W, p)
    };
    CubicPoint128 minus = {
        submod128(P.H, Q.H, p), submod128(P.X, Q.X, p),
        submod128(P.W, Q.W, p)
    };
    u128 fp = cubic_eval128(w, plus);
    u128 fm = cubic_eval128(w, minus);
    u128 c21 = halfmod128(submod128(fp, fm, p), p);
    u128 c12 = halfmod128(addmod128(fp, fm, p), p);
    if (c21 == 0 && c12 == 0) return 0;
    CubicPoint128 R = {
        submod128(mm128(c21, Q.H, m), mm128(c12, P.H, m), p),
        submod128(mm128(c21, Q.X, m), mm128(c12, P.X, m), p),
        submod128(mm128(c21, Q.W, m), mm128(c12, P.W, m), p)
    };
    if (R.H == 0 && R.W == 0) return 0;
    u128 W2 = mm128(R.W, R.W, m);
    u128 HX = mm128(R.H, R.X, m);
    u128 c = submod128(mm128(addmod128(w->v0, w->v0, p), W2, m),
                       addmod128(HX, HX, p), p);
    out->H = mm128(R.H, R.H, m);
    out->X = addmod128(mm128(R.H, R.X, m), c, p);
    out->W = mm128(R.H, R.W, m);
    return out->W != 0;
}

static int twist_random_point128(u128 *uo, u128 *vo, Rng *rng, u128 p,
                                 u128 mask, u128 eta, u128 sqrtm1_m,
                                 const Mont128 *m) {
    u128 one = m->one;
    u128 two = addmod128(one, one, p);
    u128 six = addmod128(addmod128(two, two, p), two, p);
    for (;;) {
        u128 u = toM128(rand_below128(rng, p, mask), m);
        u128 u2 = mm128(u, u, m);
        u128 f = addmod128(submod128(mm128(u2, u2, m), mm128(six, u2, m), p),
                           one, p);
        u128 v;
        if (sqrt128_domain(&v, mm128(eta, f, m), p, sqrtm1_m, m)) {
            if (rng64(rng) & 1) v = submod128(0, v, p);
            *uo = u; *vo = v; return 1;
        }
    }
}

static int nonsplit_walker_init128(NonsplitWalker128 *w, Rng *rng, u128 p,
                                   u128 mask, u128 sqrtm1_m,
                                   const Mont128 *m) {
    memset(w, 0, sizeof(*w));
    w->p = p; w->rng = rng; w->mt = m;
    u128 one = m->one;
    u128 two = addmod128(one, one, p);
    u128 eta = ((p & 3) == 3) ? submod128(0, one, p) : two;
    w->eta = eta;
    w->sqrtm1_m = sqrtm1_m;
    w->kplus = w->kminus = one;
    if ((p & 3) != 3) {
        u128 inv2 = halfmod128(one, p);
        u128 q = mm128(sqrtm1_m, inv2, m);
        if (!sqrt128_domain(&w->kplus, q, p, sqrtm1_m, m)) return 0;
        q = submod128(0, q, p);
        if (!sqrt128_domain(&w->kminus, q, p, sqrtm1_m, m)) return 0;
    }
    if (!twist_random_point128(&w->u0, &w->v0, rng, p, mask, eta,
                               sqrtm1_m, m)) return 0;
    u128 u02 = mm128(w->u0, w->u0, m);
    u128 u03 = mm128(u02, w->u0, m);
    u128 four = addmod128(two, two, p);
    u128 six = addmod128(four, two, p);
    u128 twelve = addmod128(six, six, p);
    w->c4 = eta;
    w->c3 = mm128(eta, mm128(four, w->u0, m), m);
    w->c2 = mm128(eta, submod128(mm128(six, u02, m), six, p), m);
    w->c1 = mm128(eta, submod128(mm128(four, u03, m),
                                 mm128(twelve, w->u0, m), p), m);
    for (int i = 0; i < 8; i++) {
        u128 u, v, h;
        do {
            if (!twist_random_point128(&u, &v, rng, p, mask, eta,
                                       sqrtm1_m, m)) return 0;
            h = submod128(u, w->u0, p);
        } while (h == 0);
        w->inc[i].H = mm128(h, h, m);
        w->inc[i].X = addmod128(v, w->v0, p);
        w->inc[i].W = h;
        if (cubic_eval128(w, w->inc[i]) != 0) return 0;
    }
    w->cur = w->inc[0];
    w->ready = 1;
    return 1;
...
static int nonsplit_walker_fill128(NonsplitWalker128 *w, u128 *us, u128 *vs,
                                   int n) {
    if (!w || !w->ready || n < 1 || n > 32) return 0;
    CubicPoint128 rows[32];
    u128 den[32], inv[32];
    for (int i = 0; i < n; i++) {
        int ok = 0;
        for (int tries = 0; tries < 16 && !ok; tries++) {
            unsigned j = (unsigned)(rng64(w->rng) & 7);
            CubicPoint128 next;
            int valid = cubic_add128(&next, w, w->cur, w->inc[j]);
#ifndef NDEBUG
            if (valid) valid = (cubic_eval128(w, next) == 0);
#endif
            if (valid) {
                w->cur = next;
                ok = 1;
    ...
    if (!invert_batch128_domain(inv, den, n, w->p, w->mt)) return 0;
    for (int i = 0; i < n; i++) {
        u128 h = mm128(rows[i].H, inv[i], w->mt);
        u128 xa = mm128(rows[i].X, inv[i], w->mt);
        us[i] = addmod128(w->u0, h, w->p);
        vs[i] = submod128(mm128(h, xa, w->mt), w->v0, w->p);
    }
    return n;
}

static int x16_fill_nonsplit_domain(u128 *As, u128 *xPs, u128 *deltas, int cap,
                                    Rng *rng, u128 p, u128 rand_mask,
                                    u128 sqrtm1_m, const Mont128 *mt,
                                    NonsplitWalker128 *walker) {
    enum { MAXY = 32 };
    if (cap > MAXY) cap = MAXY;
    u128 ys[MAXY], y2s[MAXY], r0s[MAXY], negfroots[MAXY];
    u128 sds[MAXY], qbs[MAXY], twoqas[MAXY], rdroots[MAXY], gfacs[MAXY];
    u128 corrections[MAXY];
    unsigned char derived[MAXY];
    u128 vals[2 * MAXY], invs[2 * MAXY];
    const u128 one = mt->one;
    const u128 two = addmod128(one, one, p);
    const u128 four = addmod128(two, two, p);
    const u128 neg1 = submod128(0, one, p);
    const u128 neg2 = submod128(0, two, p);
    u128 walk_us[MAXY], walk_vs[MAXY];
    int walk_n = 0, walk_pos = 0;
    int ny = 0;
    while (ny < cap) {
        u128 y, negfroot = 0;
        if (walker && walker->ready) {
            if (walk_pos >= walk_n) {
                walk_n = nonsplit_walker_fill128(walker, walk_us, walk_vs, cap);
                walk_pos = 0;
                if (!walk_n) return 0;
            }
            u128 u = walk_us[walk_pos];
            negfroot = walk_vs[walk_pos++];
            u128 nu = submod128(0, u, p);
            if (u > nu) u = nu;
            y = addmod128(one, u, p);
        } else {
            u128 yr = rand_below128(rng, p, rand_mask);
            if (yr == 0) continue;
            /* A(1+u)=A(1-u).  In the nonsplit family these two labels mark
               generators of the same cyclic 2-Sylow and have identical lift
               depth.  Canonicalize before any field exponentiation. */
            u128 ymate = submod128(2, yr, p);
            if (yr > ymate) continue;
            y = toM128(yr, mt);
        }
        u128 y2 = mm128(y, y, mt);
        if (!(walker && walker->ready)) {
            u128 four_y = addmod128(addmod128(y, y, p),
                                    addmod128(y, y, p), p);
            u128 f = mm128(submod128(y2, two, p),
                           addmod128(submod128(y2, four_y, p), two, p), mt);
            if (f == 0 || sqrt128_domain(&negfroot, f, p, sqrtm1_m, mt)) continue;
        }

        u128 y3 = mm128(y2, y, mt);
        u128 qa = submod128(y2, addmod128(y, y, p), p);
        if (qa == 0) continue;
        u128 qb = submod128(addmod128(y2, y2, p), y3, p);
        u128 qc = submod128(one, y, p);
        u128 twoqa = addmod128(qa, qa, p);
        u128 twoqc = addmod128(qc, qc, p);
        u128 D = submod128(mm128(qb, qb, mt), mm128(twoqa, twoqc, mt), p);
        u128 sd;
        int derive_mate_root = 0;
        u128 correction = one;
        if (!sqrt128_domain(&sd, D, p, sqrtm1_m, mt)) {
            /* With u=y-1,
                 D(u)  = (u^4-1)(u^2+2u-1),
                 D(-u) = (u^4-1)(u^2-2u-1),
               and D(u)D(-u)=(u^4-1)^2*f(u).  Since this source is
               nonsplit (chi(f)=-1), exactly one D is square.  Switch to
               the mate rather than throwing the curve away and repeating
               the nonsplit exponentiation on another random y. */
            u128 rdroot = sd;
            u128 u = submod128(y, one, p);
            u128 u2 = mm128(u, u, mt);
            u128 gfac = submod128(mm128(u2, u2, mt), one, p);
            if (walker && walker->ready && (p & 3) != 3) {
                u128 r2 = mm128(sd, sd, mt);
                u128 iD = mm128(walker->sqrtm1_m, D, mt);
                if (r2 == iD) correction = walker->kplus;
                else if (r2 == submod128(0, iD, p)) correction = walker->kminus;
                else return 0;
            }
            derive_mate_root = 1;
            y = submod128(two, y, p);
            y2 = mm128(y, y, mt);
            y3 = mm128(y2, y, mt);
            qa = submod128(y2, addmod128(y, y, p), p);
            if (qa == 0) continue;
            qb = submod128(addmod128(y2, y2, p), y3, p);
            qc = submod128(one, y, p);
            twoqa = addmod128(qa, qa, p);
            twoqc = addmod128(qc, qc, p);
            rdroots[ny] = rdroot;
            gfacs[ny] = gfac;
        }
        ys[ny] = y;
        y2s[ny] = y2;
        negfroots[ny] = negfroot;
        sds[ny] = sd;
        qbs[ny] = qb;
        twoqas[ny] = twoqa;
        corrections[ny] = correction;
        derived[ny] = (unsigned char)derive_mate_root;
        ny++;
    }

    /* Recover all conjugate D roots with one batched inversion.  If
       r_f^2=-f and r_D^2=-D(u), then
         r_f*(u^4-1)/r_D
       squares to D(-u). */
    u128 rootargs[MAXY], rootinvs[MAXY];
    int rootids[MAXY], nr = 0;
    for (int i = 0; i < ny; i++) if (derived[i]) {
        rootids[nr] = i;
        rootargs[nr++] = rdroots[i];
    }
    if (nr) {
        if (!invert_batch128_domain(rootinvs, rootargs, nr, p, mt)) return 0;
        for (int j = 0; j < nr; j++) {
            int i = rootids[j];
            u128 s = mm128(mm128(negfroots[i], gfacs[i], mt), rootinvs[j], mt);
            s = mm128(s, corrections[i], mt);
            u128 qc = submod128(one, ys[i], p);
            u128 Dmate = submod128(mm128(qbs[i], qbs[i], mt),
                                   mm128(twoqas[i], addmod128(qc, qc, p), mt), p);
            u128 s2 = mm128(s, s, mt);
            if (s2 != Dmate && (p & 3) != 3) {
                s = mm128(s, sqrtm1_m, mt);
                s2 = mm128(s, s, mt);
            }
            if (s2 != Dmate) return 0;
            sds[i] = s;
        }
    }

    for (int i = 0; i < ny; i++) {
        u128 r0 = submod128(sds[i], qbs[i], p);
        u128 ym1 = submod128(ys[i], one, p);
        u128 ym1_2 = mm128(ym1, ym1, mt);
        u128 denA = mm128(four, mm128(ym1_2, ym1_2, mt), mt);
        u128 dx0 = submod128(r0, mm128(twoqas[i], ys[i], mt), p);
        if (denA == 0 || dx0 == 0) return 0;
        r0s[i] = r0;
        vals[2 * i] = denA;
        vals[2 * i + 1] = dx0;
    }
    if (!invert_batch128_domain(invs, vals, 2 * ny, p, mt)) return 0;
    int nout = 0;
    for (int i = 0; i < ny; i++) {
        u128 A = mm128(x16_A_num_domain(ys[i], p, mt), invs[2 * i], mt);
        if (A == 0 || A == one || A == two || A == neg1 || A == neg2) continue;
        u128 xp = mm128(r0s[i], invs[2 * i + 1], mt);
        if (xp == 0) continue;
        As[nout] = A;
        xPs[nout] = xp;
        if (deltas) {
            if ((p & 3) == 3) {
                /* A^2-4 = f * (y^2*G(y)/(4*(y-1)^4))^2,
                   G=y^4-6y^3+14y^2-16y+8.  negfroot^2=-f. */
                u128 c4 = four;
                u128 c8 = addmod128(c4, c4, p);
                u128 c6 = addmod128(c4, two, p);
                u128 c14 = addmod128(addmod128(c8, c4, p), two, p);
                u128 c16 = addmod128(c8, c8, p);
                u128 G = one;
                G = submod128(mm128(G, ys[i], mt), c6, p);
                G = addmod128(mm128(G, ys[i], mt), c14, p);
                G = submod128(mm128(G, ys[i], mt), c16, p);
                G = addmod128(mm128(G, ys[i], mt), c8, p);
                u128 ratio = mm128(mm128(y2s[i], G, mt), invs[2 * i], mt);
                deltas[nout] = mm128(negfroots[i], ratio, mt);
            } else {
                deltas[nout] = 0;
            }
        }
...
static int halve_once_first_domain(u128 *xo, u128 p, u128 A, u128 x,
                                   u128 sqrtm1_m, const Mont128 *mt) {
    const u128 one = mt->one;
    const u128 two = addmod128(one, one, p);
    const u128 four = addmod128(two, two, p);
    u128 d = addmod128(addmod128(mm128(x, x, mt), mm128(A, x, mt), p), one, p);
    u128 sd;
    if (!sqrt128_domain(&sd, d, p, sqrtm1_m, mt)) return 0;
    u128 roots[2] = { sd, submod128(0, sd, p) };
    for (int i = 0; i < 2; i++) {
        u128 u = addmod128(addmod128(x, x, p),
                           addmod128(roots[i], roots[i], p), p);
        u128 w = submod128(mm128(u, u, mt), four, p);
        u128 sw;
        if (!sqrt128_domain(&sw, w, p, sqrtm1_m, mt)) continue;
        u128 v = addmod128(u, sw, p);
        u128 candidate = (v & 1) ? ((v + p) >> 1) : (v >> 1);
        if (candidate != 0) { *xo = candidate; return 1; }
    }
    return 0;
}

static int halve_chain_domain(u128 *xout, u128 p, u128 A, u128 x,
                              int depth, int k, u128 sqrtm1_m,
                              const Mont128 *mt) {
    for (; depth < k; depth++) {
        if (!halve_once_first_domain(&x, p, A, x, sqrtm1_m, mt)) return 0;
    }
    u128 An = frM128(A, mt);
    u128 xn = frM128(x, mt);
    if (!verify128(p, An, xn)) return 0;
    *xout = xn;
    return 1;
}

/* p=3 mod 4 nonsplit halving with a cached delta=sqrt(4-A^2).  If the
   first W is nonsquare, its failed root R satisfies R^2=-W.  The conjugate
   branch root is 4*X*Z*delta/R; the formulas below absorb R into the new
   projective denominator, so no inversion or second exponentiation occurs. */
static int halve_chain_projective_domain(u128 *xout, u128 p, u128 A, u128 x,
                                         u128 delta, int depth, int k,
                                         const Mont128 *mt) {
    const u128 one = mt->one;
    const u128 two = addmod128(one, one, p);
    const u128 four = addmod128(two, two, p);
    u128 X = x, Z = one;
    for (; depth < k; depth++) {
        u128 X2 = mm128(X, X, mt);
        u128 Z2 = mm128(Z, Z, mt);
        u128 XZ = mm128(X, Z, mt);
        u128 D = addmod128(addmod128(X2, mm128(A, XZ, mt), p), Z2, p);
        u128 S;
        if (!sqrt128_domain(&S, D, p, 0, mt)) return 0;

        u128 Up = addmod128(addmod128(X, X, p), addmod128(S, S, p), p);
        u128 fourZ2 = mm128(four, Z2, mt);
        u128 Wp = submod128(mm128(Up, Up, mt), fourZ2, p);
        u128 T;
        if (sqrt128_domain(&T, Wp, p, 0, mt)) {
            X = addmod128(Up, T, p);
            Z = addmod128(Z, Z, p);
        } else {
            /* T^2=-Wp here. */
            u128 Um = submod128(addmod128(X, X, p), addmod128(S, S, p), p);
            u128 fourXZdelta = mm128(four, mm128(XZ, delta, mt), mt);
            X = addmod128(mm128(Um, T, mt), fourXZdelta, p);
            Z = mm128(addmod128(Z, Z, p), T, mt);
        }
        if (X == 0 || Z == 0) return 0;
    }
    u128 zinv = pow128_domain(Z, p - 2, mt);
    u128 xn = frM128(mm128(X, zinv, mt), mt);
    u128 An = frM128(A, mt);
    if (!verify128(p, An, xn)) return 0;
    *xout = xn;
    return 1;
...
/* Test a packet of marked 16-torsion points breadth-first.  Return the packet
   index of a verified order-2^k point, or -1.  xs is updated in place. */
static int halve_chains_batch128(u128 *As, u128 *xs, int n, u128 p,
                                 int start_depth, int k, u128 sqrtm1,
                                 const Mont128 *mt) {
    unsigned char alive[64];
    int ids[64], ids2[64];
    u128 args[64], roots[64], sds[64], us[64];
    unsigned char ok[64];
    for (int i = 0; i < n; i++) alive[i] = 1;

    for (int depth = start_depth; depth < k; depth++) {
        int m = 0;
        for (int i = 0; i < n; i++) if (alive[i]) {
            ids[m] = i;
            u128 x2 = mulmod128_mont(xs[i], xs[i], mt);
            args[m] = addmod128(addmod128(x2, mulmod128_mont(As[i], xs[i], mt), p), 1, p);
            m++;
        }
        if (m == 0) return -1;
        sqrtmod128_batch(roots, ok, args, m, p, sqrtm1, mt);

        int mw = 0;
        for (int j = 0; j < m; j++) {
            int i = ids[j];
            if (!ok[j]) { alive[i] = 0; continue; }
            sds[i] = roots[j];
            u128 u = addmod128(addmod128(xs[i], xs[i], p),
                               addmod128(roots[j], roots[j], p), p);
            us[i] = u;
            ids2[mw] = i;
            args[mw] = submod128(mulmod128_mont(u, u, mt), 4, p);
            mw++;
        }
        sqrtmod128_batch(roots, ok, args, mw, p, sqrtm1, mt);

        int mf = 0;
        for (int j = 0; j < mw; j++) {
            int i = ids2[j];
            if (ok[j]) {
                u128 v = addmod128(us[i], roots[j], p);
                xs[i] = (v & 1) ? ((v + p) >> 1) : (v >> 1);
            } else {
                /* Try the other sign of sqrt(d), exactly as the scalar
                   first-branch routine does. */
                u128 nsd = submod128(0, sds[i], p);
                u128 u = addmod128(addmod128(xs[i], xs[i], p),
                                   addmod128(nsd, nsd, p), p);
                us[i] = u;
                ids[mf] = i;
                args[mf] = submod128(mulmod128_mont(u, u, mt), 4, p);
                mf++;
            }
        }
        sqrtmod128_batch(roots, ok, args, mf, p, sqrtm1, mt);
        for (int j = 0; j < mf; j++) {
            int i = ids[j];
            if (!ok[j]) { alive[i] = 0; continue; }
            u128 v = addmod128(us[i], roots[j], p);
    ...
    for (int i = 0; i < n; i++) {
        if (alive[i] && verify128(p, As[i], xs[i])) return i;
    }
    return -1;
}
...
static int search128(u128 p) {
    ...
    u128 sqrtm1_m = sqrtm1 ? toM128(sqrtm1, &mt) : 0;
...
#pragma omp parallel
    {
        ...
        for (int i=0;i<200;i++) rng64(&rng);
        NonsplitWalker128 walker;
        NonsplitWalker128 *walkerp = NULL;
        if (g_use_walker &&
            nonsplit_walker_init128(&walker, &rng, p, rand_mask,
                                    sqrtm1_m, &mt))
            walkerp = &walker;
        u64 budget = max_trials / nthr + 1, lc = 0;
        u128 pending_A = 0, pending_xP = 0;
        int have_pending_A = 0;
        u128 x16_batch_A[32], x16_batch_x[32], x16_batch_delta[32];
        int x16_batch_pos = 0, x16_batch_count = 0;

        while (!found && lc < budget) {
            if (0 && g_x16_mode && g_x16_halve_mode &&
                g_x16_nonsplit_filter_mode && !g_x16_first_d_skip_mode &&
                !g_x16_first_d_y_gate_mode && !g_x16_halve_full_mode) {
                int bn = x16_fill_nonsplit_batch128(
                    x16_batch_A, x16_batch_x, 32,
                    &rng, p, rand_mask, sqrtm1, &mt);
                int hi = halve_chains_batch128(x16_batch_A, x16_batch_x, bn,
                                               p, 4, k, sqrtm1, &mt);
                lc += (u64)bn;
                if (hi >= 0) {
#pragma omp critical
                    {
                        if (!found) {
                            found = 1;
                            found_A = x16_batch_A[hi];
                            found_x0 = x16_batch_x[hi];
                            double el = now_sec() - t0;
            ...
            if (g_x16_mode) {
                if (g_x16_first_d_skip_mode) {
                    ...
                    if (x16_batch_pos >= x16_batch_count) {
                        x16_batch_count = x16_fill_nonsplit_domain(
                            x16_batch_A, x16_batch_x, x16_batch_delta, 32,
                            &rng, p, rand_mask, sqrtm1_m, &mt, walkerp);
                        x16_batch_pos = 0;
                    }
                    A = x16_batch_A[x16_batch_pos];
                    xP16 = x16_batch_x[x16_batch_pos++];
                } else {
                    x16_montgomery_A128(&A, &xP16, &pending_A, &pending_xP, &have_pending_A,
                                         &rng, p, rand_mask, sqrtm1, &mt);
                }
            ...
            if (g_x16_halve_mode) {
                u128 xR;
                int domain_path = g_x16_nonsplit_filter_mode &&
                                  !g_x16_first_d_skip_mode &&
                                  !g_x16_first_d_y_gate_mode &&
                                  !g_x16_halve_full_mode;
                int hit;
                if (domain_path && (p & 3) == 3 && !g_force_affine) {
                    hit = halve_chain_projective_domain(
                        &xR, p, A, xP16, x16_batch_delta[x16_batch_pos - 1],
                        x_start_depth, k, &mt);
                } else if (domain_path) {
                    hit = halve_chain_domain(&xR, p, A, xP16, x_start_depth,
                                             k, sqrtm1_m, &mt);
                } else {
                    hit = halve_chain_from_depth128(&xR, p, A, xP16,
                                                    x_start_depth, k,
                                                    sqrtm1, &mt);
                }
                if (hit) {
#pragma omp critical
                    {
                        if (!found) {
                            found=1;
                            found_A = domain_path ? frM128(A, &mt) : A;
                            found_x0=xR;
                            double el = now_sec()-t0;
                            printf("Found after %.2fs (~%llu X1(16) curves)\n\n",
                                   el, (unsigned long long)(lc * nthr));
...
#pragma omp critical
        ...
        if (tid >= 0 && tid < 256) thread_counts[tid] = lc;
    }
    ...
    if (found) {
        ...
        printf("\n\n");
        int valid = verify128(p, found_A, found_x0);
        printf("Verified: %s  (%.2fs)\n", valid?"PASS":"FAIL", elapsed);
        if (!valid) return 1;
        u64 tested = 0;
        for (int i = 0; i < 256; i++) tested += thread_counts[i];
        if (g_answer_fd >= 0) {
            char ps[50], as[50], xs[50], line[180];
            sprint128(ps, p); sprint128(as, found_A); sprint128(xs, found_x0);
            int len = snprintf(line, sizeof(line), "%s %s %llu\n",
                               as, xs, (unsigned long long)tested);
            if (len > 0) (void)write(g_answer_fd, line, (size_t)len);
        }
    } else {
        printf("Not found in %.2fs. Re-run or increase budget.\n", elapsed);
    }
...
static int quota_threads(void) {
    FILE *f = fopen("/sys/fs/cgroup/cpu.max", "r");
    if (!f) return 4;
    char quota[32];
    unsigned long long period = 0;
    int ok = fscanf(f, "%31s %llu", quota, &period);
    fclose(f);
    if (ok != 2 || period == 0 || strcmp(quota, "max") == 0) return 4;
    unsigned long long q = strtoull(quota, NULL, 10);
    unsigned long long n = (q + period - 1) / period;
    if (n < 1) n = 1;
    if (n > 64) n = 64;
    return (int)n;
}
...
int main(int argc, char **argv) {
    /* Preserve the contract stream and send inherited research diagnostics
       to stderr.  A verified hit is written directly to the saved fd. */
    g_answer_fd = dup(STDOUT_FILENO);
    if (g_answer_fd >= 0) (void)dup2(STDERR_FILENO, STDOUT_FILENO);
    g_domain_window3 = getenv("POM_WINDOW3") != NULL;
    g_domain_slide3 = getenv("POM_BINARY") == NULL;
    g_force_affine = getenv("POM_AFFINE") != NULL;
    g_use_walker = getenv("POM_NO_WALKER") == NULL;
#ifdef _OPENMP
    omp_set_dynamic(0);
    const char *thread_override = getenv("POM_THREADS");
    omp_set_num_threads(thread_override ? atoi(thread_override) : quota_threads());
#endif
    if (argc < 2) {
